 ...
 */
public class Table <TCol extends ITableColumn> extends AbstractControl implements ITable <TCol> {
	 ...
	 * @param locator
	 */
	public Table (final IContainer parentContainer, final By locator, final Class <TCol> cls) {
		super (parentContainer, locator);
		this.rows = finds (By.tagName ("tr")).stream ().filter (e -> e.isDisplayed ())
				.map (e -> new TableRow<> (driver (), e, cls)).collect (Collectors.toList ());
	}
	...
	public ITableRow <TCol> row (final int index) {
		if (index < 0 || index >= this.rows.size ()) { return null; }
